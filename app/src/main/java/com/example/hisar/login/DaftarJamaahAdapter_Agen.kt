...
class DaftarJamaahAdapter_Agen(private var data: ArrayList<Jamaah.DataItem>, private val legth:Int):
RecyclerView.Adapter<DaftarJamaahAdapter_Agen.ViewHolder>(){
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): DaftarJamaahAdapter_Agen.ViewHolder {
    ...
    override fun onBindViewHolder(holder: DaftarJamaahAdapter_Agen.ViewHolder, position: Int) {
        val item = data[position]
        holder.bind(item)
        holder.open.setOnClickListener {
            val intent = Intent(holder.itemView.context,ProfileJamaah_Agen::class.java)
                .putExtra("id",item.id)
